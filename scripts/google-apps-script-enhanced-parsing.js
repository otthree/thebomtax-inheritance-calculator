// Google Apps Script 코드 - 상세 계산 결과 파싱 버전
...
function doPost(e) {
  try {
    ...
    // 정규식으로 계산 결과 추출 (상세 버전)
    const totalAssetsMatch = inquiry.match(/총 재산가액:\s*([\d,]+)원/)
    const totalDebtMatch = inquiry.match(/총 채무:\s*([\d,]+)원/)
    const finalTaxMatch = inquiry.match(/최종 상속세:\s*([\d,]+)원/)

    // O/X 형태로 공제 정보 추출
    const basicDeductionMatch = inquiry.match(/일괄공제:\s*([OX])/)
    const spouseDeductionMatch = inquiry.match(/배우자공제:\s*([OX])/)
    const housingDeductionMatch = inquiry.match(/동거주택공제:\s*([OX])/)
